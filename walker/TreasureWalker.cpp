 ...
 */
TreasureWalker::TreasureWalker(Uptime* uptime, LineMonitor* lineMonitor)
    : mUptime(uptime),
...
void TreasureWalker::reset(const ScenarioParams& params) {
...
void TreasureWalker::setParam(double speed, double kp, double kd, double ki) {
    mBaseSpeed = speed;
    mSteeringCoef = kp;
...
void TreasureWalker::getParam(double* target, double* speed, double* kp, double* kd, double* ki) {
...
void TreasureWalker::clearIntegral() {
    mIntegral = 0.0;
}
...
Control TreasureWalker::get() { return Control(mBaseSpeed, mSteeringAmount); }